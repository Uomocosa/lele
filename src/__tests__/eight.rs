...
#[tokio::test]
// run test by using: 'cargo test __tests__::eight::test -- --exact --nocapture'
...
pub async fn eight() -> Result<()> {
    ...
    let user1 = create_user_and_join(topic_id, addrs.clone()).await?;
    ...
    println!("> closing user1 ..."); user1.close().await?;
    println!("> closing user2 ..."); user2.close().await?;
    Ok(())
}
